...
import { BudgetOverviewResponse } from '../../services/api/financeApi';
import { useAppDispatch, useAppSelector } from '../../store/hooks';
import { loadBudgetOverview, setPeriod } from '../../store/slices/budgetSlice';
...
export default function BudgetOverviewScreen({ navigation }: any) {
  ...
  const selectedMonth = useAppSelector(state => state.budget.selectedMonth);
  const selectedYear = useAppSelector(state => state.budget.selectedYear);
  const overview = useAppSelector(state => state.budget.overview);
  ...
  useEffect(() => {
    dispatch(loadBudgetOverview({ month: selectedMonth, year: selectedYear }));
  ...
  const spentPercentage = overview?.totalAmount
    ? Math.round((overview.totalSpent / overview.totalAmount) * 100)
    : 0;
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}>
          ...
          <Text style={styles.subtitle}>Plan smart. Spend better.</Text>
        </View>
        <TouchableOpacity
          ...
          onPress={() =>
            Alert.alert(
              'Manage your budget',
              'Set a monthly budget, assign amounts to categories, and track your spending here. Tap a category to view its transactions.',
            )
          }
        ...
        {loading ? (
          ...
          <View style={styles.emptyState}>
            <View style={styles.emptyIcon}>
              <Icon name="calendar" size={54} color={colors.primary} />
            </View>
            <Text style={styles.emptyTitle}>No Budget Set</Text>
            <Text style={styles.emptyDescription}>
              {error
                ? 'Unable to load budget details right now.'
                : `You haven't set any budget for ${selectedMonthLabel}.`}
            </Text>
          </View>
        ) : (
          <View>
            ...
            <View style={styles.statsRow}>
              <Stat
                ...
                note={`${spentPercentage}%`}
                color={colors.primary}
              />
              <Stat
                label="Remaining"
                value={overview.totalRemaining}
                note={`${100 - spentPercentage}%`}
            ...
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity
                onPress={() => navigation.navigate('ViewAllCategoryBudget')}
              >
...
function renderCategoryCard(
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
      onPress={() =>
        navigation.navigate('BudgetCategoryOverview', {
